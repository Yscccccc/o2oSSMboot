...
public class QuartzConfiguration {
	...
	@Autowired
	private CronTriggerFactoryBean prductSellDailyTriggerFactory;
	...
	public MethodInvokingJobDetailFactoryBean createJobDetail() {
		...
		//由于我们所做的定时任务根本上 就是执行一个方法，所以用这个工厂比较方便
		...
		//指定concurrent设为false,多个job不会并发运行，第二个job将不会再第一个job完成之前开始
	...
	@Bean(name = "prductSellDailyTriggerFactory")
	public CronTriggerFactoryBean createPrductSellDailyTrigger() {
	...
	public SchedulerFactoryBean createSchedulerFactory() {
		...
		schedulerFactory.setTriggers(prductSellDailyTriggerFactory.getObject());
