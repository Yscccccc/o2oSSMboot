$(function(){
	...
	var pageSize = 10;
	//列出商品列表的url
	var listUrl = '/o2oboot/frontend/listproductsbyshop';
	...
	var searchDivUrl = '/o2oboot/frontend/listshopdetailpageinfo?shopId=' + shopId;
	...
	function getSearchDivData(){
		...
		$.getJSON(url, function(data){
			if (data.success) {
				...
				//将商品类别a标签绑定到下相应的html组件中
				$('#shopdetail-button-div').html(html);
			}
		});
	}
	//渲染出店铺基本信息以及商品类别列表一共搜索
	getSearchDivData();
	/**
	 * 获取分页展示的商品列表信息
	 */
	function addItems(pageSize, pageIndex){
		...
		//设动加载符，若还在后台取数据则不能再次访问后台，避免多次重复加载
		loading = true;
		//访问后台获取相应查询条件下的商品列表
		$.getJSON(url, function(data){
			//获取当前查询条件下商品的总数
			...
			//获取到目前为止已显示的卡片总数，包含之前已经加载的
			var total = $('.list-div .card').length;
			//若总数达到按照此查询条件列出来的总数一致，则停止后台的加载
			if (total >= maxItems) {
...
//				$.detachInfiniteScroll($('.infinite-scroll'));
	...
	$('.list-div').on('click','.card',function(e) {
		...
		window.location.href = '/o2oboot/frontend/productdetail?productId='
				+ productId;
	});
