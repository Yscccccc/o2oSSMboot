$(function(){
	...
	//根据主键获取授权信息的URl
	var infoUrl = '/o2oboot/shopadmin/getshopauthmapbyid?shopAuthId=' + shopAuthId;
	//修改授权信息的URL
	var shopAuthPostUrl = '/o2oboot/shopadmin/modifyshopauthmap';
	
	if (shopAuthId) {
		getInfo(shopAuthId);
	} else {
		$.toast("用户不存在！");
		window.location.href = '/o2oboot/shopadmin/shopmanagement';
	}
		...
		$.ajax({
			...
			success: function(data){
				if(data.success){
					$.toast("提交成功！");
					$('#captcha_img').click();
					window.location.href = '/o2oboot/shopadmin/shopauthmanagement';
				} else{
					$.toast("提交失败！");
					$('#captcha_img').click();
				}
